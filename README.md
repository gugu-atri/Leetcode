...
## 食用说明~（重要）
>
- 这个笔记适合什么人呢，最好是一刷了hot100或者有算法基础的人。因为它不像市面上一堆的教程那样，事无巨细解析加举一反三加普及算法知识。这种教程不好吗？当然很好，写的也很用心。但是对于我来说这样学习的有点慢，最主要的是目的不同，我的目的是**应付面试**，所以对算法的追求并不高，短时间掌握为主。
- 我一刷时也是咬文嚼字深究题解，每道题都先酣畅淋漓自己写最后看题解发现一两h过去，浪费了时间也没掌握(指三天后忘记)，我想说的是：我认为最适合刷hot100的姿势就是理解加背，然后推荐看docx版本和pdf版本完整连贯一些。
- 笔记大致流程是题目说明-示例-思路加关键实现-参考图。题目按照hot100官方顺序分类排序，原则有几个：1解法是挑尽量简单好理解背的方法，能ac就行不强求击败100；2困难题基本都跳过，我根据解法难度和评论判断，如果解法复杂难背或者评论没提到面试情况，pass了，精力不能浪费在这里；3思路和关键实现的说明能短点就短点，长了不利于背诵吟唱。所以我说最好一刷过或者有基础的同学来看
- 目前只有java写法，写完再后考虑加入py和c等。有其他选手愿意参与可以提pr(墓前看，包沉底的);
- 最后大家看得开心，可以转载~
